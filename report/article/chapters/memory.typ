...
In order to increase our performance, we adopted a data oriented design programming approach by increasing memory cache efficiency. As matrices are the largest data structures of our programms, we had to understand how they were stored.
...
This means when we acces elements of a matrix, we should iterate over elements from the same columns as they will already be loaded in the cache, resulting in faster load times.
...
Finally we combined some linear algebra operations to remove temporary matrices.
```cpp 

void rank1Update(const Vector<T>& vec1, const Vector<T>& vec2, T scalar) {
    for (size_t j = 0; j < m_cols; ++j) {
        T factor = scalar * vec2(j);
        for (size_t i = 0; i < m_rows; ++i) {
